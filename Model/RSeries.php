...
class RSeries extends AppModel {
    ...
    public function findRangeSeries($fromSeStartYmd, $gradeOnly){
    	...
    	if(empty($fromSeStartYmd)){
    		...
    		$sql = "select min(SE_START_YMD) as TO_SE_START_YMD, max(SE_START_YMD) as FROM_SE_START_YMD from (select SE_START_YMD  from R_SERIES where SE_DAYS = END_DAYS and SE_START_YMD < ?" . $whereGrade . " order by SE_START_YMD desc limit 20) T";
    		$params = array($fromSeStartYmd);
    		$result = $this->query($sql, $params);
    	}
